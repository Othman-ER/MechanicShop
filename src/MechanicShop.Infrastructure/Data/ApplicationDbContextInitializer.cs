    ...
    RoleManager<IdentityRole> roleManager)
{
    ...
    public async Task TrySeedAsync()
    {
        ...
        if (roleManager.Roles.All(r => r.Name != laborRole.Name))
        ...
        {
            await userManager.CreateAsync(manager, manager.Email);

            if (!string.IsNullOrWhiteSpace(managerRole.Name))
                await userManager.AddToRolesAsync(manager, [managerRole.Name]);
        }

        var labor01 = new ApplicationUser
        {
        ...
        {
            await userManager.CreateAsync(labor01, labor01.Email);

            if (!string.IsNullOrWhiteSpace(laborRole.Name))
                await userManager.AddToRolesAsync(labor01, [laborRole.Name]);
        }

        var labor02 = new ApplicationUser
        {
        ...
        {
            await userManager.CreateAsync(labor02, labor02.Email);

            if (!string.IsNullOrWhiteSpace(laborRole.Name))
                await userManager.AddToRolesAsync(labor02, [laborRole.Name]);
        }

        var labor03 = new ApplicationUser
        {
        ...
        {
            await userManager.CreateAsync(labor03, labor03.Email);

            if (!string.IsNullOrWhiteSpace(laborRole.Name))
                await userManager.AddToRolesAsync(labor03, [laborRole.Name]);
        }

        var labor04 = new ApplicationUser
        {
        ...
        {
            await userManager.CreateAsync(labor04, labor04.Email);

            if (!string.IsNullOrWhiteSpace(laborRole.Name))
                await userManager.AddToRolesAsync(labor04, [laborRole.Name]);
        }
        ...
        if (!context.Customers.Any())
        {
            ...
            [
                ...
                Vehicle.Create(
                    ...
                    model: "Civic",
                    year: 2018,
                    licensePlate: "XYZ456").Value,
            ];

            List<Vehicle> vehicles02 =
            [
                Vehicle.Create(
            ...
            [
                ...
                Customer.Create(
                    ...
                    vehicles: vehicles02).Value,
            ]);
        }

        if (!context.RepairTasks.Any())
        {
                ...
                RepairTask.Create(
                    ...
                    [
                        Part.Create(
                            ...
                            "Engine Oil", 25.00m, 1).Value, 
                        Part.Create(
                            Guid.Parse("62ad80e3-2cff-41af-ab40-16fab8db8b38"), 
                            "Oil Filter", 10.00m, 1).Value
                    ]).Value,
        ...
        {
            var repairTasks = context.RepairTasks.ToList();
            ...
            var repairTasksForFirstOrder = context.RepairTasks
             .OrderBy(_ => Guid.NewGuid())
             .Take(2)
             .ToList();
            ...
            var workOrderStartingNow = WorkOrder.Create(
                Guid.NewGuid(),
                context.Vehicles.OrderBy(_ => Guid.NewGuid()).First().Id,
                startTimeFirstOrder,
            ...
            var repairTasksEndingNow = context.RepairTasks
      .First(rt => rt.EstimatedDurationInMins == RepairDurationInMinutes.Min60);
            ...
            context.WorkOrders.AddRange(generatedWorkOrders);
