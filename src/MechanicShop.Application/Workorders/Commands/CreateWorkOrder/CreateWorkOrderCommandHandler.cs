    ...
    IRequestHandler<CreateWorkOrderCommand, Result<WorkOrderDto>>
{
    public async Task<Result<WorkOrderDto>> Handle(CreateWorkOrderCommand command, CancellationToken ct)
    {
        ...
        if (workOrderPolicy.IsOutsideOperatingHours(command.StartAt, totalEstimatedDuration))
        {
        ...
        var checkMinRequirementResult = workOrderPolicy.ValidateMinimumRequirement(command.StartAt, endAt);
